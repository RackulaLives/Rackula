<script lang="ts">
  ...
  import { parseLayoutYaml, parseYaml, serializeLayoutToYaml } from "$lib/utils/yaml";
  ...
  async function handleReloadLatest(): Promise<void> {
    const latest = latestYamlAtConflict ?? (await serializeLayoutToYaml(layout));
    baselineYaml = latest;
    yamlText = latest;
    showConflictPrompt = false;
    latestYamlAtConflict = null;
  ...
  {#if showConflictPrompt}
    <div class="conflict-box" role="alert" data-testid="yaml-conflict-prompt">
      ...
      <p class="conflict-copy">
        Choose how to proceed to avoid overwriting newer layout changes by mistake.
      </p>
