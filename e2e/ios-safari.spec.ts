 ...
 */
async function setupMobileViewport(page: Page, device: (typeof iosDevices)[number]) {
...
test.describe('FAB Button (Device Library)', () => {
	...
	test('FAB is NOT visible on iPad Pro 12.9 (desktop mode)', async ({ page }) => {
...
test.describe('Device Label Positioning', () => {
	for (const device of mobileDevices.slice(0, 2)) {
		test(device.name + ' - device labels render within bounds', async ({ page }) => {
			await setupMobileViewport(page, device);
