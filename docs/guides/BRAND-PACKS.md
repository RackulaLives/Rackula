...
- `slot_width: 1` marks a half-width device; `u_height` below 1 marks sub-U gear. Both mount inside a carrier rather than directly on the rails, which Rackula handles at placement time (see the fit rules in [SPEC.md](../reference/SPEC.md#mounting-model)). Do not set `width_mm` in a brand pack: the 2.0 schema stamp reads only a layout's own device types, so a shipped measured device would save stamped 1.1, and `brandpacks.test.ts` fails on it. `width_mm` is for custom devices users create.
