...
- `slot_width: 1` marks a half-width device; `width_mm` gives a measured width in millimetres for gear narrower than that (for example a mini PC); `u_height` below 1 marks sub-U gear. All of these mount inside a carrier rather than directly on the rails, which Rackula handles at placement time.
