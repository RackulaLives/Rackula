...
| Preview URLs | On (`preview_urls: true`, `workers_dev: false`) | Bot Fight Mode challenges CI on every racku.la host, so curl checks need the workers.dev preview URL. The API there has no Access JWT and fails closed (401) |
...
- `/api/layouts` on the preview URL returns 401: proves the API fails closed without Access.
