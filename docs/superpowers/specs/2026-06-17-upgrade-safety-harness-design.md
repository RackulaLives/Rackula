...
- Recurring current-format fixtures (the per-release captures from Component 4): the sidecar is auto-generated by a script that loads the fixture through the current pipeline and records the result. Dropping a new fixture is one command with zero hand-authoring.
