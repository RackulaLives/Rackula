...
Run: `npm run test:run -- src/tests/effective-face.test.ts`
Expected: FAIL (cannot resolve `$lib/utils/effective-face`).
...
Run: `npm run test:run -- src/tests/collision-full-depth.test.ts`
Expected: FAIL on the first test (`canPlaceDevice(... "rear")` returns `true`, expected `false`).
...
Run: `npm run test:run -- src/tests/collision`
Expected: PASS (no regressions in existing collision tests).
...
Run: `npm run test:run -- src/tests/rear-view-full-depth.test.ts`
Expected: FAIL on the new test (`Unable to find an element with the text: NAS`); the existing two tests still pass.
...
Expected: PASS (whole suite green, including the existing blocked-slots and export tests).
...
Run: `npm run test:run -- src/tests/annotation-full-depth.test.ts`
Expected: FAIL (`Unable to find an element with the text: Big NAS`).
...
Run: `npm run test:run -- src/tests/rear-treatment.test.ts`
Expected: FAIL on the first test (`Unable to find an element with the text: REAR`).
...
Run: `npm run test:run -- src/tests/empty-face-hint.test.ts`
Expected: FAIL on the first test (hint text not found).
...
Run: `npm run test:run -- src/tests/full-depth-face-guard.test.ts`
Expected: FAIL on the first test (face stored as `"front"`, expected `"both"`).
...
Run: `npm run lint`
Expected: no errors. In particular, confirm no new test triggers the `no-restricted-syntax` rules (querySelector, toHaveClass, toHaveLength literal, hardcoded colours).
