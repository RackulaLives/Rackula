...
- the CVE id, or the GHSA id for an advisory without one
...
`deploy-dev.yml` smokes the dev Worker twice, like prod. Before any traffic moves, `scripts/smoke-headers.sh --surface dev` and the Playwright boot smoke run against the per-version preview URL; the curl smoke also asserts that `/api/layouts` answers 404 there. Preview URLs sit outside Cloudflare Access and outlive their version, so the Worker locks the API to d.racku.la (`RACKULA_API_HOST`) and no preview URL can reach R2. After promotion, the Playwright smoke runs against d.racku.la with the Access service token (`CF_ACCESS_CLIENT_ID` / `CF_ACCESS_CLIENT_SECRET`, #2346) and checks `/api/layouts` through Access. A missing token fails the run rather than skipping it: the deploy job only runs on `main` of this repository, where the secrets exist, and skips for forks and for dispatches from any other ref.
