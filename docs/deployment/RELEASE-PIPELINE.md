...
`deploy-dev.yml` smokes the dev Worker twice, like prod. Before any traffic moves, `scripts/smoke-headers.sh --surface dev` runs against the per-version preview URL and also asserts that `/api/layouts` answers 401 there, because the preview URL is outside Cloudflare Access and the API must fail closed. After promotion, the Playwright smoke runs against d.racku.la with the Access service token (`CF_ACCESS_CLIENT_ID` / `CF_ACCESS_CLIENT_SECRET`, #2346) and checks `/api/layouts` through Access. A missing token fails the run rather than skipping it: Deploy Dev only runs on this repository, where the secrets exist.
